...
class CourseController extends Controller
{
    ...
    public function view($slug){
        ...
        $related_courses = Course::where('parent_category_id',$course->parent_category_id)->where('status','1')->whereNotIn('id', [$course->id, 0])->take(3)->get();
        $units = course_subject::where('course_id',$course->id)->get();
        ...
        return view(theme('course'), compact('course', 'title', 'isEnrolled','units','related_courses','discounts','status_flag'));
    }
    ...
    public function store(Request $request){
        ...
        $data = [
            ...
            'meta_dis'          => $request->meta_description,
    ...
    public function informationPost( Request $request, $course_id){
        ...
        $data = [
            ...
            'online_workshops'       => clean_html($request->online_workshops),
            'included'          => clean_html($request->included),
            'assessment'       => clean_html($request->assessment),
            'duration'          => clean_html($request->duration),
            'assessment'       => clean_html($request->assessment),
            'progression'          => clean_html($request->progression),
            'entry_requirements'          => clean_html($request->entry_requirements),
           // 'requirements'      => clean_html($request->requirements),
           'y_gain_qualification'          => clean_html($request->y_gain_qualification),
            'thumbnail_id'      => $request->thumbnail_id,
            'category_id'       => $request->topic_id,
            'parent_category_id' => $category->category_id,
            'second_category_id' => $category->id,
    ...
    public function pricingSet(Request $request,  $course_id){
        ...
        $data = [
            ...
            'rfee'        => clean_html($request->rfee),
            'afee'        => clean_html($request->afee),
    ...
    public function payment($payment_id,$course_id){
        $title = "Payment";
        $user = Auth::user();
        $payment_details = Payment::find($payment_id);
        $invoice_details = StudentInvoice::where('payment_id',$payment_id)->where('course_id',$course_id)->get();

        return view(theme('dashboard/payment'), compact('title','payment_details','invoice_details','course_id'));
    }
