...
class Payment extends Model
{
    ...
    protected $fillable = [
        'name','email','user_id','amount','total_amount','fees_name','fees_amount','fees_total','fees_type','payment_method','status','currency', 'token_id',
        'card_last4','card_id','card_brand', 'card_country','card_exp_month', 'card_exp_year','client_ip','charge_id_or_token','payer_email', 'description',
        'local_transaction_id','payment_created','bank_swift_code', 'account_number','branch_name','branch_address','account_name', 'iban','payment_note',
        'created_at', 'updated_at','support','terms_accept'
    ];
    ...
    public static function update_and_sync($data,$payment_id){
    
        $payment = Payment::where('id',$payment_id)->update($data);
  
        // $enrolls = Enroll::where('payment_id',$payment_id)->update(['status'=>'success']);
        DB::table('enrolls')->where('payment_id', $payment_id)->update(['status' => 'success']);
    ...
    public function studentinvoice(){
        return $this->hasMany(StudentInvoice::class, 'payment_id');
    }
}
