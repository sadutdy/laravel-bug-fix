...
$(function() {
        ...
        $.post(pageData.routes.remove_cart, {cart_id : cart_id, _token : pageData.csrf_token}, function(response){
            if (response.success){
               ...
               $("#mydiv").load(location.href + " #mydiv");
                if(pURL.indexOf('checkout') != -1){
                   // location.reload();
                   $(".shop-checkouts ").load(location.href + " .shop-checkouts");
                }
               
               // $('.mini-cart-item-checkout').reload();
                                //$("[data=value]").html(response.cart_html);
            }
        });
    });
    ...
    $(document).on('click', '.add-to-cart-btn', function (e) {
        ...
        var payment_type = $btn.attr('data-payment-type');
        ...
        $.ajax({
            ...
            type : 'POST',
            data : {course_id : course_id,payment_type:payment_type,_token : pageData.csrf_token},
            beforeSend: function () {
                $btn.addClass('loader').attr('disabled', 'disabled');
            },
            success: function (response) {
    ...
    $("#listSearch").on("keyup", function() {
        ...
        $(".courses_group .top_courses .smlcourse-block_img-wrap--course-name").each(function() {
      ...
      $("#blended_toggle").click(function() {
        $("#collapseFour").show();
        $("#collapseThree").hide();
     
    });

   $('#options #filter').find('li:first').trigger("click" );

//    $('#masonry_abc .information-technology').css("display", "none");
  
   $('#masonry_abc .management').css("display", "none");
   $('#masonry_abc .business').css("display", "none");
   $('#masonry_abc .health-and-social-care').css("display", "none");
   $('#masonry_abc .consulting').css("display", "none");
   $('#masonry_abc .teaching').css("display", "none");
   $('#masonry_abc .hotel-management').css("display", "none");
   $('#masonry_abc .public-relation').css("display", "none");
   $('#masonry_abc .marketing').css("display", "none");
//    $('#masonry_abc .tourism-and-hospitality').css("display", "none");
  

$("#stripecustomCheck00").on("click", function(){
    var check;
    check = $("#stripecustomCheck00").prop("checked");
    if(check) {
        $.ajax({
            url : '/getAddress',
            type : 'POST',
            ...
            success: function (response) {
              ...
              $('#stripe_name').val(data.name);
              $('#stripe_email').val(data.user_email);
              $('#stripe_contact').val(data.user_phone);
              $('#stripe_address').val(data.user_address);
              $('#stripe_street').val(data.user_address_2);
              $('#stripe_city').val(data.user_city);
              $('#stripe_postcode').val(data.user_postcode);
              $("#stripe_country").val(data.user_country).change();
              $('#stripe_country option[value='+data.user_country+']').prop("selected", true);
            //   $('select[name="stripe_country"]').val(data.user_country);
